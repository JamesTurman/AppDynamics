...
ProtobufModel.prototype.init = function() {
  ...
  self.userCodeErrRegEx = /at\s([^\:]+)\:(\d+)\:\d+$/;
  self.classRegex = /^(.+)\.([^\.]+)$/;
    ...
    if(stackTraceStr) {
      ...
      lines.forEach(function(line) {
        ...
        if(traceMatch && traceMatch.length == 4) {
          ...
          if (!(traceMatch[2].indexOf("node_modules/appdynamics") > -1)) {
        ...
        } else if (userCodeErrMatch && userCodeErrMatch.length == 3) {
          if (!(userCodeErrMatch[1].indexOf("node_modules/appdynamics") > -1)) {
            stackTrace.elements.push({
              klass: '',
              method: '',
              fileName: userCodeErrMatch[1],
              lineNumber: parseInt(userCodeErrMatch[2])
            });
          }
        }
      });
