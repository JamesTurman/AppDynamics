...
LibAgent.prototype.readNodeIndex = function(callback) {
  ...
  fs.exists(self.agent.indexDir, function(exists) {
    if (!exists) return;

    fs.readdir(self.agent.indexDir, function(err, indexFiles) {
      if (err) return self.agent.logger.error(err);

      indexFiles.forEach(function(indexFile) {
        var nodeIndex = parseInt(indexFile.split('.')[0]);
        if (!isNaN(nodeIndex)) {
          fs.readFile(self.agent.indexDir + '/' + indexFile, function(err, pid) {
            if (err) return self.agent.logger.error(err);
...
LibAgent.prototype.initializeLogger = function() {
  this.agent.libagentConnector.init();
  this.agent.emit('agentInitialized');
  this.agent.logger.setLibAgentConnector(this.agent.libagentConnector);
  this.agent.logger.init(this.agent.opts.logging, false);
};
...
LibAgent.prototype.startAgent = function(metadata) {
  this.agent.emit('agentStarted', metadata);
};
