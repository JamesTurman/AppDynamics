...
MongodbProbe.prototype.attach = function(obj) {
  ...
  if ('instrument' in obj) {
    ...
    self.agent.on('destroy', function() {
      if(obj.__appdynamicsProbeAttached__) {
        ...
        ['cursor', 'insert', 'update', 'remove'].forEach(function(command) {
          proxy.release(obj.Server.prototype[command]);
          proxy.release(obj.ReplSet.prototype[command]);
        });
      }
    });
    ...
    listener.on('started', function(event) {
      ...
      serverPool = nextRequest ? nextRequest.serverPool : serverPool;
      ...
      if (serverPool && serverPool.length && (nextRequest || opQueue[requestId])) {
        if (nextRequest) {
          opQueue[requestId] = {
          ...
          nextRequest = null;
        } else if (!opQueue[requestId]) return;
        request = opQueue[requestId];
        var commandArgs = {
          databaseName: event.databaseName,
          collectionName: event.command[event.commandName],
          query: profiler.truncate(JSON.stringify(event.command.filter)),
    ...
    listener.on('failed', function(event) {
      ...
      if (request) self.addExitCall(event.time, event.exitCall, event.failure);
    });
