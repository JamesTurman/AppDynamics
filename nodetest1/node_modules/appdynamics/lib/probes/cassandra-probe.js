...
CassandraProbe.prototype.attach = function(obj) {
  ...
  // recieve a callback so we need both before and after
  // advice to handle both cases
  proxy.around(obj.Client.prototype, ['execute', 'batch'],
    before, after
  );
  ...
  // callback as it's preceeded by another (non-optional)
  // function argument
  ...
  function before(obj, args, locals, noCallback) {
    ...
    if (noCallback) {
      ...
      self.agent.proxy.callback(args, -1, function(obj, args) {
...
function createExitCall(client, profiler, time, command, params, props) {
  ...
  return profiler.createExitCall(time, {
    ...
    command: profiler.truncate(stringifiedCommand),
